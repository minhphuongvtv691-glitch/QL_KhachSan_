...
export async function renderInvoices(container, session) {
    container.innerHTML = `
        ...
        <!-- Summary bar -->
        <div id="staff-invoices-summary" style="display:flex;gap:16px;margin-bottom:24px;"></div>
        <!-- Lọc dữ liệu -->
    ...
    const renderTable = () => {
        ...
        pageData.forEach(i => {
            ...
            const bookingColHTML = `staff` === 'admin' 
...
const getFilters = () => {
    ...
    const loadData = async () => {
        ...
        try {
            ...
            const summaryBar = document.getElementById("staff-invoices-summary");
            if (summaryBar) {
    ...
    // Xử lý refresh
    const containerEl = document.getElementById("staff-invoices-summary")?.parentElement;
    if (containerEl) {
